...
/* ----- Webolite AI Chat ----- */
(function () {
  var fab = document.getElementById('wbChatFab');
  var panel = document.getElementById('wbChatPanel');
  var closeBtn = document.getElementById('wbChatClose');
  var form = document.getElementById('wbChatForm');
  var input = document.getElementById('wbChatInput');
  var messages = document.getElementById('wbChatMessages');
  var suggestions = document.getElementById('wbChatSuggestions');
  var root = document.getElementById('weboliteChat');
  if (!fab || !panel) return;
  ...
  function openChat() {
    ...
    if (!greeted) {
      greeted = true;
      addMsg('bot', "Hi! I know this whole site — services, pricing, process, location, and more. Ask me anything about Webolite.");
    }
  ...
  function addTyping() {
    var div = document.createElement('div');
    div.className = 'wb-msg bot typing';
  ...
  async function sendQuestion(q) {
    ...
    try {
      ...
      if (res.ok && data.reply) {
        ...
        addMsg('bot', data.error || "Sorry, I couldn't answer that right now. Try the contact form or email hello@webolite.com.");
      }
    } catch (e) {
      typing.remove();
      addMsg('bot', "I'm offline while the server isn't running. Start the server with python3 server.py, or use the contact form.");
    }
  }
  ...
  if (suggestions) {
    suggestions.querySelectorAll('button').forEach(function (btn) {
      btn.addEventListener('click', function () {
        sendQuestion(btn.getAttribute('data-q') || btn.textContent);
      });
    });
  }
})();
